...
void CVHaarWavelet(Mat &src,Mat &dst,int NIter)
{
	...
	int width = src.cols;
	int height = src.rows;	//src.at(row, col);
	for (int k=0;k<NIter;k++) 
	{
		for (int y=0;y<(height>>(k+1));y++)
		{
			...
			{
				c=(src.at<float>(2*y,2*x)+src.at<float>(2*y,2*x+1)+src.at<float>(2*y+1,2*x)+src.at<float>(2*y+1,2*x+1))*0.5;	//average
				dst.at<float>(y,x)=c;

				dh=(src.at<float>(2*y,2*x)+src.at<float>(2*y+1,2*x)-src.at<float>(2*y,2*x+1)-src.at<float>(2*y+1,2*x+1))*0.5;	//¥ª¥k®t
				dst.at<float>(y,x+(width>>(k+1)))=dh;

				dv=(src.at<float>(2*y,2*x)+src.at<float>(2*y,2*x+1)-src.at<float>(2*y+1,2*x)-src.at<float>(2*y+1,2*x+1))*0.5;	//¤W¤U®t
...
void CVInvHaarWavelet(Mat &src,Mat &dst,int NIter, int SHRINKAGE_TYPE, float SHRINKAGE_T)
{
	...
	{
		for (int y=0;y<(height>>k);y++)	// height / (2^k)
		{
...
int Main_HarrWavelet()
{
	...
	Mat mat = imread(str);
	Mat grayMat = Mat(mat.rows, mat.cols, CV_8UC1);
	cvtColor(mat, grayMat, CV_BGR2GRAY);
	IplImage iplGrayImg = grayMat;
	cvShowImage("gray.bmp", &iplGrayImg);
	Mat Src = Mat(mat.rows, mat.cols, CV_32FC1);
	Mat Dst = Mat(mat.rows, mat.cols, CV_32FC1);
	Mat Temp = Mat(mat.rows, mat.cols, CV_32FC1);
	Mat Filtered = Mat(mat.rows, mat.cols, CV_32FC1);
	grayMat.convertTo(Src,CV_32FC1);
	
	const int NIter=3;	//wavelet iterate number
	Dst=0;
	CVHaarWavelet(Src,Dst,NIter);
	...
	if((M-m)>0) 
	{
		...
		//Dst = Dst * (2550.0/(M-m))-m/(M-m);	//for enhance show
	}
	IplImage iplCoeffImg = Dst;
	cvShowImage("coeff.bmp", &iplCoeffImg);
	waitKey(0);
	...
	if((M-m)>0) 
	{
		...
		//Filtered = Filtered * (2550.0/(M-m))-m/(M-m);	//for enhance show
	}        
	IplImage iplFilterImg = Filtered;
	cvShowImage("filtered.bmp", &iplFilterImg);
	waitKey(0);
	return 0;
}